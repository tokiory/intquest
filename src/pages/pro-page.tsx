...
import { ProFeature, ProFeatureImage, type ProFeatureImageProperties, type ProFeatureProperties } from "@/components/ProFeature";

type CardsType = {type: "description"} & ProFeatureProperties | {type: "image"} & ProFeatureImageProperties
...
export const ProPage = () => {
  ...
  const freeVersionComparsionList = [
    ...
    { accessible: true, text: "Ссылки на полезные материалы к каждому вопросу" },
    {
      accessible: true,
      limited: true,
  ...
  return (
    <PageWrapper>
      <Title className="flex gap-2 justify-center mb-12 text-center">Intquest Pro <Icon icon="fluent-emoji:sparkles"/></Title>
      <div className="grid grid-cols-5 gap-3 w-full">
        {cards.map((card, index) => (
          card.type === "description" ? <ProFeature key={index} {...card} /> : <ProFeatureImage key={index} {...card} />
        ))}
      </div>
      <div className="grid grid-cols-2 gap-3 mt-12 w-full">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Все что нужно для самоподготовки к собеседованиям</CardDescription>
          </CardHeader>
          <CardContent>
            <ProComparsionList list={freeVersionComparsionList} />
            <div className="flex flex-col gap-2 mt-6">
              <Text className="text-xs"><sup>*</sup> Возможно просмотреть не более 50 ответов на вопросы в месяц</Text>
              <Text className="text-xs"><sup>*</sup> Возможно отправить не более 3 коллекций вопросов в месяц</Text>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Все для найма, проверки грейдов и управления списками вопросов</CardDescription>
          </CardHeader>
          <CardContent>
